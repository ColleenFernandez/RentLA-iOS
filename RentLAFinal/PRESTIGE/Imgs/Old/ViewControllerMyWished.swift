...
class ViewControllerMyWished: UIViewController,UITableViewDelegate,UITableViewDataSource,UITabBarDelegate,customProtocol  {
    ...
    @IBAction func clickNew(_ sender: Any) {
        ...
        if let viewController = UIStoryboard(name: "\(UserDefaults.standard.string(forKey: "STORY_BOARD")!)", bundle: nil).instantiateViewController(withIdentifier: "ViewControllerNewPost") as? ViewControllerNewPost
                                       {
    ...
    func numberOfSections(in tableView: UITableView) -> Int
            ...
            func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
            {
               ...
               func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
               {
                   ...
                   let cell:PostCell = self.tblView.dequeueReusableCell(withIdentifier: "PostCell") as! PostCell
                ...
                if indexPath.row % 2 == 0 {
                    ...
                    cell.imageObj.image = UIImage.init(named: "ann3.jpg")
                    cell.lblPName.text = "Anniversary John & Juna"
                  
                } else {
                 
                    cell.imageObj.image = UIImage.init(named: "bir.jpg")
                     cell.lblPName.text = "Birthday Peterson"
                }
                ...
                cell.btnCommend.addTarget(self, action: #selector(self.cmdClicked(_:)), for: .touchUpInside)
                          cell.btnCommend.tag = (indexPath.section * 1000) + indexPath.row
                 */
                   return cell as PostCell
               }
          
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        
        if let viewController = UIStoryboard(name: "\(UserDefaults.standard.string(forKey: "STORY_BOARD")!)", bundle: nil).instantiateViewController(withIdentifier: "ViewControllerWishedComment") as? ViewControllerWishedComment
                                              {
                                                  if let navigator = navigationController
                                                  {
                                                      navigator.pushViewController(viewController, animated: true)
    ...
    @objc func cmdClicked(_ button: UIButton?) {
       ...
       print("You selected Row #\(row)! Section #\(section)!")
           ...
           if let viewController = UIStoryboard(name: "\(UserDefaults.standard.string(forKey: "STORY_BOARD")!)", bundle: nil).instantiateViewController(withIdentifier: "ViewControllerComments") as? ViewControllerComments
           {
               if let navigator = navigationController
               {
                   navigator.pushViewController(viewController, animated: true)
